...
- Tests: runnable self-checks in `tests/`, no pytest. `uv run python tests/test_barge_in.py` and `tests/test_barge_command.py` are plain asserts; `tests/test_playback.py` and `tests/test_idle.py` use stdlib unittest. Some are slow to start (importing `speech_to_speech.py` pulls in transformers + textual).
