...
- Run: `uv run speech_to_speech.py` (Textual TUI; `t` focuses the text input, Esc unfocuses it. Quitting is menu-only: top menu bar `System` → `(Q)uit` → confirm dialog. `q`/`Ctrl+Q` do NOT quit.) `hello.py` is the `uv init` scaffold — ignore it.
