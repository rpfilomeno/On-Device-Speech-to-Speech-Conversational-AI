...
- Tests: runnable self-checks in `tests/`, no pytest. `uv run python tests/test_barge_in.py`, `tests/test_barge_command.py`, `tests/test_history_trim.py`, and `tests/test_text_history_trim.py` are plain asserts; `tests/test_playback.py` and `tests/test_idle.py` use stdlib unittest. Some are slow to start (importing `speech_to_speech.py` pulls in transformers + textual). `test_playback.py` is flaky: it fails when autotuned `settings.json` values (`TARGET_SIZE: 6`, `PLAYBACK_DELAY`) don't match its assumptions — run it against a clean `settings.json`.
...
- `settings.json` (repo root) overrides env for `MIC_DEVICE`, `SPEAKER_DEVICE`, `TARGET_SIZE`, `PLAYBACK_DELAY`; set via `/config` or `save_settings()`. It is committed; don't treat it as scratch.
...
- `speech_to_speech.py` holds the TUI, slash-command dispatch, idle loop, and adaptive playback-pacing logic (some tests import it directly). `calibrate_barge.py` is the barge calibration harness.
- `text_to_speech.py` is a **thin entrypoint** for the text-only variant (no mic/VAD/Whisper; text in → Kokoro TTS out). Its logic lives in `src/text_to_speech/`: `state.py` (event bus + shared mutable state), `logging.py` (error routing), `history.py` (context-budget trimming), `pacing.py` (adaptive chunking + jitter learners), `pipeline.py`, `tui.py`. It never imports `speech_to_speech.py`.
