...
- Run: `uv run speech_to_speech.py` (main interactive app, Ctrl+C to stop) or `uv run text_to_speech.py` (single-turn TTS demo). `hello.py` is the `uv init` scaffold — ignore it.
...
- Linux: eSpeak NG must be installed (`apt install espeak-ng`) for fallback TTS; the `keyboard` lib needs root.
