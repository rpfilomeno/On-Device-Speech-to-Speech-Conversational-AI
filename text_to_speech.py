...
from src.utils.config import settings, save_settings, log_error
...
def pipeline_main():
    ...
    try:
        ...
        emit("status", "LISTENING")
        emit("log", "=== Ready. Type below to chat (spoken reply via TTS). ===")
        ...
        while not shutdown_event.is_set():
            if pause_event.is_set():
                if not was_paused:
                    emit("status", "PAUSED")
                    emit("log", "[Command] Paused: voice output and idle countdown suspended. /play to resume.")
                was_paused = True
                time.sleep(0.1)
                continue
            if was_paused:
